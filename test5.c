...
int main(int argc, char* argv[]){
	...
	// allocate
	void *p0 = alloc_and_set(1);printf("TEST 0 PASSED - ALLOCATED\n");
	check(p0,1);
	void *p1 = alloc_and_set(2);printf("TEST 1 PASSED - ALLOCATED\n");
	check(p1,2);
	void *p2 = alloc_and_set(3);printf("TEST 2 PASSED - ALLOCATED\n");
	check(p2,3);
	void *p3 = alloc_and_set(4);printf("TEST 3 PASSED - ALLOCATED\n");
	check(p3,4);
	void *p4 = alloc_and_set(5);printf("TEST 4 PASSED - ALLOCATED\n");
	check(p4,5);
	void *p5 = alloc_and_set(6);printf("TEST 5 PASSED - ALLOCATED\n");
	check(p5,6);
	
	//free
	myfree(p0);
	printf("TEST 6 PASSED - FREED\n");
	myfree(p2);
	printf("TEST 7 PASSED - FREED\n");
	myfree(p4);
	printf("TEST 8 PASSED - FREED\n");
